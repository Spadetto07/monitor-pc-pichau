...
def coletar(page, alvo: dict) -> dict:
    ...
    resposta = page.goto(BASE_URL + alvo["slug"], wait_until="load", timeout=60000)
    ...
    try:
        ...
        # Sem o preco na tela o motivo importa: bloqueio, desafio de bot, pagina
        # de erro ou mudanca de layout dao sintomas diferentes. Sem diagnostico o
        # log so diz "timeout" e nao da para agir.
        try:
            corpo = (page.locator("body").text_content() or "")[:400]
        except Exception:
            corpo = "<sem body>"
        raise RuntimeError(
            f"preco nao renderizou | HTTP {status} | url final {page.url} | "
            f"titulo {page.title()!r} | inicio do body: {' '.join(corpo.split())!r}"
        )
